...
// Role context for login (which app is being used)
...
class AuthMobileService {
   ...
   */
  async sendCode(input: SendCodeInput): Promise<{ message: string; expiresIn: number }> {
  ...
  async verifyCode(input: VerifyCodeInput): Promise<AuthResponse> {
    ...
    const loginRole: LoginRole = (role as LoginRole) || 'client';
  ...
  ): Promise<{ id: string; phone: string; name: string | null; language: string }> {
   ...
   */
  async getProfile(userId: string, role: LoginRole = 'client') {
    const user = await prisma.users.findUnique({
      where: { id: userId },
      include: {
        client_profiles: true,
