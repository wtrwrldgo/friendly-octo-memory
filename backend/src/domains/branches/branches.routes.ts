...
router.get(
  '/:id',
  validate({ params: branchIdSchema }),
  branchesController.getById.bind(branchesController)
);

router.get(
  '/firm/:firmId',
  validate({ params: z.object({ firmId: z.string().uuid() }) }),
  branchesController.getByFirmId.bind(branchesController)
);
...
router.post(
  '/',
  authenticate,
  requireRoles(UserRole.WATERGO_ADMIN, UserRole.FIRM_OWNER),
...
router.delete(
  ...
  requireRoles(UserRole.WATERGO_ADMIN, UserRole.FIRM_OWNER),
  validate({ params: branchIdSchema }),
  branchesController.delete.bind(branchesController)
);
