...
interface ClientListParams {
  firmId?: string;
  branchId?: string;
  includeGlobal?: boolean; // Only for WaterGo Super Admin
}
...
export const clientsService = {
   ...
   */
  async listFirmClients(firmId: string, branchId?: string) {
    // Build order filter for firm and optionally branch
    const orderFilter: Record<string, unknown> = { firm_id: firmId };
    if (branchId) {
  ...
  async listGlobalClients() {
    ...
    return clients.map((client: any) => {
      ...
      const uniqueFirmIds = new Set(client.orders.map((o: any) => o.firm_id));
   ...
   */
  async getById(id: string, firmId?: string) {
