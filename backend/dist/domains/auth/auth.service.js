...
class AuthService {
    ...
    async driverLogin(input) {
        // Find driver directly by ID using raw SQL (DB schema differs from Prisma)
        const drivers = await prisma.$queryRawUnsafe(`SELECT d.id, d.firm_id, d.driver_number, d.vehicle_number, f.name as firm_name
       FROM drivers d
       LEFT JOIN firms f ON d.firm_id = f.id
       WHERE d.id::text = $1
       LIMIT 1`, input.driverId);
        ...
        const token = jwt.sign({ driverId: driver.id, role: 'DRIVER', firmId: driver.firm_id }, config.jwt.secret, { expiresIn: config.jwt.expiresIn });
        return {
            token,
            driver: {
                id: driver.id,
    ...
    async adminLogin(input) {
        ...
        const token = jwt.sign({ id: staff.id, email: staff.email, role: 'ADMIN' }, config.jwt.secret, { expiresIn: config.jwt.expiresIn });
        return {
            token,
            admin: {
                id: staff.id,
