...
export function requireFirmAccess(req, _res, next) {
    if (!req.user) {
        next(new UnauthorizedError('Authentication required'));
        return;
    }
...
export function requireBranchAccess(req, _res, next) {
    if (!req.user) {
        next(new UnauthorizedError('Authentication required'));
        return;
    }
...
export function isFirmOwner(req) {
    return req.user?.role === UserRole.FIRM_OWNER;
}
// Helper to check if user can manage firm
export function canManageFirm(req, firmId) {
    if (!req.user)
        return false;
    if (req.user.role === UserRole.WATERGO_ADMIN)
        return true;
    if (req.user.role === UserRole.FIRM_OWNER && req.user.firmId === firmId)
        return true;
    return false;
}
