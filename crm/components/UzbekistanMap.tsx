...
export default function UzbekistanMap({
  ...
  // Calculate label positions based on bounding box of path
  const getLabelPosition = (regionId: string): { x: number; y: number } => {
    const positions: Record<string, { x: number; y: number }> = {
      karakalpakstan: { x: 130, y: 180 },
      xorazm: { x: 235, y: 265 },
      bukhara: { x: 360, y: 350 },
      navoiy: { x: 420, y: 220 },
      samarqand: { x: 500, y: 360 },
      qashqadaryo: { x: 480, y: 430 },
      surxondaryo: { x: 530, y: 490 },
      jizzakh: { x: 545, y: 340 },
      sirdaryo: { x: 580, y: 315 },
      tashkent_region: { x: 630, y: 270 },
      tashkent_city: { x: 612, y: 272 },
      namangan: { x: 700, y: 280 },
      fergana: { x: 695, y: 330 },
      andijan: { x: 755, y: 305 },
    };
  ...
  return (
    <div className="relative w-full">
      ...
      >
        {/* Background */}
        <rect width="793" height="520" fill="#F8FAFC" rx="16" />

        {/* Draw all regions */}
        {UZBEKISTAN_REGIONS.map((region) => {
          const isSelected = selectedRegion === region.id;
          const isHovered = hoveredRegion === region.id;
          const branchCount = branchCounts[region.id] || 0;
          ...
          return (
            <g key={region.id}>
              ...
              <path
                ...
                fill={
                  isSelected
                    ...
                    ? MAP_COLORS.hover
                    : hasBranches
                    ? MAP_COLORS.withBranches
                    : MAP_COLORS.noBranches
                }
                stroke={isSelected ? MAP_COLORS.selectedStroke : MAP_COLORS.stroke}
              ...
              {region.id !== "tashkent_city" && (
                <text
                  x={getLabelPosition(region.id).x}
                  y={getLabelPosition(region.id).y}
                  textAnchor="middle"
                  className="pointer-events-none select-none"
                  style={{
                    fontSize: region.id === "karakalpakstan" ? "11px" : "9px",
              ...
              {/* Branch count badge */}
              {hasBranches && !isSelected && (
                <g>
                  <circle
                    cx={getLabelPosition(region.id).x + 30}
                    cy={getLabelPosition(region.id).y - 12}
                    r="10"
                    fill="#22C55E"
                  />
                  <text
                    x={getLabelPosition(region.id).x + 30}
                    y={getLabelPosition(region.id).y - 8}
                    textAnchor="middle"
                    style={{ fontSize: "8px", fontWeight: 700, fill: "#FFFFFF" }}
        ...
        <g transform="translate(20, 460)">
          <rect width="200" height="50" fill="white" rx="8" opacity="0.9" />
          <circle cx="20" cy="18" r="8" fill="#3B82F6" stroke="#D1D5DB" />
          <text x="35" y="22" style={{ fontSize: "10px", fill: "#64748B" }}>
            Есть филиалы
          </text>
          <circle cx="20" cy="38" r="8" fill="#F3F4F6" stroke="#D1D5DB" />
          <text x="35" y="42" style={{ fontSize: "10px", fill: "#64748B" }}>
            Нет филиалов
          </text>
          <circle cx="130" cy="18" r="8" fill="#1D4ED8" stroke="#1E40AF" />
          <text x="145" y="22" style={{ fontSize: "10px", fill: "#64748B" }}>
            Выбрано
          </text>
        </g>
      </svg>
      ...
      {hoveredRegion && hoveredRegion !== selectedRegion && (
        <div className="absolute top-4 right-4 bg-white dark:bg-gray-800 px-4 py-2 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
          <p className="font-semibold text-navy-900 dark:text-white">
            {UZBEKISTAN_REGIONS.find((r) => r.id === hoveredRegion)?.nameRu}
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Филиалов: {branchCounts[hoveredRegion] || 0}
          </p>
        </div>
      )}
    </div>
